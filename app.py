...
def make_quick_choices(labels: List[str]) -> List[str]:
    """12個に丸める（互換のため残すが、基本は assemble_quick を使用）"""
    return labels[:MAX_QUICKREPLIES]


def assemble_quick(options: List[str], controls: List[str] | Tuple[str, ...] = ()) -> List[str]:
    """
    LINEのQuickReplyは最大13個程度（端末差あり）。
    オプション + コントロール（やり直す/終了 など）を安全に合計12個以内へ丸める。
    """
...
def clip_label(label: str, maxlen: int = 20) -> str:
    """QuickReplyのlabelは20文字以内。超える場合は末尾に『…』を付けて丸める。"""
    if len(label) <= maxlen:
        return label
...
" + ASK_TASK, "quick": assemble_quick(ALL_UNIQUE["作業名"], ["終了"])}
...
if LINE_AVAILABLE and CHANNEL_ACCESS_TOKEN and CHANNEL_SECRET:
    ...
    async def callback(request: Request):
        # 1) まずボディを読み、Verify専用(空events)なら即200で返す
        ...
        if isinstance(data, dict) and data.get("events") == []:
            # LINEの Verify は {"events": []} を送る → ここで超速200
            return PlainTextResponse("OK", status_code=200)

        # 2) 通常イベントは署名検証してディスパッチ
