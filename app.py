...
変更点（v2.1 → v2.2）
- QuickReply 個数を常に上限内に丸める assemble_quick を全返答で適用
- QuickReply の label を 20 文字以内に丸める clip_label を実装
- LINE 側は表示「1. ラベル」/ 送信テキストは数値のみ、で確実に数値選択に統一
- 1対1/グループ/ルーム別の堅牢なセッションキー
- 文字化け防止の UTF-8 JSON レスポンス
- 候補は CSV 実在値のみ、出現順で提示
- 改行はすべて "\n" で統一（実改行混入の構文エラー防止）
...
APP_VERSION = "app.py (simple v2.2)"
